...
impl FromStr for Listen {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let s = s.trim();
        if !s.contains(':') {
            anyhow::bail!("`{s}` is not a listen address: use an IP address with a port or :port");
        }
        if let Some(port) = s.strip_prefix(':') {
            let port: u16 = port.parse().map_err(|_| {
                anyhow::anyhow!("`{s}` has an invalid port: use an IP address with a port or :port")
            })?;
            return Ok(Listen::Tcp(SocketAddr::from((Ipv4Addr::UNSPECIFIED, port))));
        }
        s.parse::<SocketAddr>().map(Listen::Tcp).map_err(|_| {
            anyhow::anyhow!("`{s}` is not a listen address: use an IP address with a port or :port")
        })
    }
}
